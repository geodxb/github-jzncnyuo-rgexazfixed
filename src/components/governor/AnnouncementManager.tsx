      ...
      const mockAnnouncements: Announcement[] = [
        {
          ...
          message: 'Scheduled maintenance will occur this weekend from 2:00 AM to 6:00 AM EST.',
          type: 'maintenance',
          priority: 'high',
          targetRoles: ['admin', 'investor'],
          isActive: true,
          createdBy: user?.id || '',
          createdByName: user?.name || 'Governor',
          createdAt: new Date(),
          updatedAt: new Date()
        }
      ];
      ...
      setAnnouncements(prev => 
        prev.map(ann => 
          ann.id === announcementId 
            ? { ...ann, isActive: !ann.isActive, updatedAt: new Date() }
            : ann
        )
      );
    } catch (error) {
